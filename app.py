...
def render_transfers_page(analyst):
    ...
    recent = analyst.df[analyst.df["Category"].isin(["Rent", "Other Income"])].sort_values("Timestamp", ascending=False).head(5)
    for _, row in recent.iterrows():
        st.markdown(f"""<div class="txn-row"><span>{row['Category']} - {row['Timestamp'].strftime('%d %b %Y')}</span>
        <span>{'+' if row['Type']=='Credit' else '-'}Rs.{row['Amount']:,.0f}</span></div>""", unsafe_allow_html=True)


def render_bills_page(analyst, breakdown_1mo):
    st.markdown("#### Upcoming Bills (Demo)")
    st.caption("Simulated bill list generated from your recurring spending categories.")
    bill_categories = ["Utilities", "Subscriptions"]
    for cat in bill_categories:
        amt = breakdown_1mo.get(cat, 0)
        if amt > 0:
...
def render_investments_page(analyst, breakdown_1mo):
    ...
    invest_this_month = breakdown_1mo.get("Investment", 0)
    c1, c2 = st.columns(2)
    c1.metric("Invested This Month", f"Rs.{invest_this_month:,.0f}")
    ...
    with st.container(border=True):
        ...
        def run_strategist(question=None):
            if not st.session_state.api_key:
                st.session_state.chat_history.append(("assistant_error", "Please add your free Gemini API key in the sidebar first."))
                return
            try:
                strategist = WealthStrategist(api_key=st.session_state.api_key)
                ...
                st.session_state.chat_history.append(("assistant_error", f"Error calling the AI model: {e}"))
...
def render_app():
    ...
    with st.sidebar:
        ...
        # Build (or reuse) the vector DB of instrument docs once per API key.
        # Rebuilding on every rerun would mean an embedding call per
        # instrument per interaction - unnecessary and slow.
        if st.session_state.api_key and st.session_state.kb_key_used != st.session_state.api_key:
            try:
